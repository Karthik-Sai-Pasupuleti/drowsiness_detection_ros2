...
RESPOND WITH JSON ONLY:"""
    ...
    def _publish_alert(self, alert_data: dict, camera_data: dict, carla_data: dict):
        ...
        try:
            ...
            # Point this to your mounted data directory
            save_dir = "/root/ws/drowsiness_data/llm_decisions"
            os.makedirs(save_dir, exist_ok=True)
        
            # Create a unique filename based on the timestamp
            timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"llm_decision_{self.driver_id}_{timestamp_str}.json"
            filepath = os.path.join(save_dir, filename)
        
            with open(filepath, "w") as f:
                json.dump(alert_json, f, indent=4)
            
            self.get_logger().info(f"Saved LLM decision to: {filepath}")
            # ----------------------------------------
